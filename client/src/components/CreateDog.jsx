...
const CreateDog = () => {
  ...
  const handleChangeImage = (e) => {
    ...
    if (input.image.length) {
      indexImg = input.image[input.image.length - 1].index + 1;
    } else {
      indexImg = 0;
    }
    let newImgsToState = readmultifiles(e, indexImg);
    let newImgsState = [...input.image, ...newImgsToState];
    setInput({
      name: input.name,
      weight: input.weight,
      bred_for: input.bred_for,
      breed_group: input.breed_group,
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    if (!(input.name && input.weight && input.origin && input.temperaments.length)) {
      ...
      <div className={style.container}>
        <form onSubmit={handleSubmit}>
          ...
          <div className={style.wrap2}>
            ...
            <label>Image:</label>
            <input type="file" name="image" accept="image/*" onChange={handleChangeImage} disabled={input.image.length} />
